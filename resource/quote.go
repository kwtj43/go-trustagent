...
// }
// type TpmQuoteRequest struct { // remove dup
...
type TpmQuoteContext struct {
	//	tpmQuoteResponse *taModel.TpmQuoteResponse // this needs to be refactored
	tpm tpmprovider.TpmProvider
	cfg *config.TrustAgentConfiguration
}

func NewTpmQuoteContext(cfg *config.TrustAgentConfiguration) (*TpmQuoteContext, error) {
	...
	return &TpmQuoteContext{
		cfg: cfg,
		tpm: tpm,
	}, nil
}

func (ctx *TpmQuoteContext) Close() {
	ctx.tpm.Close()
}

func (ctx *TpmQuoteContext) CreateTpmQuoteResponse(tpmQuoteRequest *taModel.TpmQuoteRequest) (*taModel.TpmQuoteResponse, error) {
	...
	for i, pcrBank := range tpmQuoteRequest.PcrBanks {
		isActive, err := ctx.tpm.IsPcrBankActive(pcrBank)
		if !isActive {
	...
	tpmQuoteResponse, err := ctx.createTpmQuote(tpmQuoteRequest)
	if err != nil {
...
func (ctx *TpmQuoteContext) getNonce(tpmQuoteRequest *taModel.TpmQuoteRequest, assetTag string) ([]byte, error) {
	log.Trace("resource/quote:getNonce() Entering")
	defer log.Trace("resource/quote:getNonce() Leaving")

	log.Debugf("resource/quote:getNonce() Received HVS nonce '%s', raw[%s]", base64.StdEncoding.EncodeToString(tpmQuoteRequest.Nonce), hex.EncodeToString(tpmQuoteRequest.Nonce))
...
func (ctx *TpmQuoteContext) readAikAsBase64() (string, error) {
...
func (ctx *TpmQuoteContext) readEventLog() (string, error) {
...
func (ctx *TpmQuoteContext) getQuote(tpmQuoteRequest *taModel.TpmQuoteRequest, nonce []byte) (string, error) {
...
func (ctx *TpmQuoteContext) getTcbMeasurements() ([]string, error) {
	...
	for _, file := range fileInfo {
		if filepath.Ext(file.Name()) == ".xml" {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "resource/quote:getTcbMeasurements() Error reading manifest file %s", file.Name())
			}
...
func (ctx *TpmQuoteContext) getAssetTags() (string, error) {
...
func (ctx *TpmQuoteContext) createTpmQuote(tpmQuoteRequest *taModel.TpmQuoteRequest) (*taModel.TpmQuoteResponse, error) {
	...
	tpmQuoteResponse.AssetTag, err = ctx.getAssetTags()
	if err != nil {
		return nil, errors.Wrap(err, "resource/quote:createTpmQuote() Error while retrieving asset tags")
	}
	...
	nonce, err := ctx.getNonce(tpmQuoteRequest, tpmQuoteResponse.AssetTag)
	if err != nil {
		return nil, errors.Wrap(err, "resource/quote:createTpmQuote() Error while generting nonce")
	}

	log.Infof("NONCE: %+v", nonce)

	// get the quote from tpmprovider
	tpmQuoteResponse.Quote, err = ctx.getQuote(tpmQuoteRequest, nonce)
	if err != nil {
		return nil, errors.Wrap(err, "resource/quote:createTpmQuote() Error while retrieving tpm quote request")
	}

	// aik --> read from disk and convert to PEM string
	tpmQuoteResponse.Aik, err = ctx.readAikAsBase64()
	if err != nil {
		return nil, errors.Wrap(err, "resource/quote:createTpmQuote() Error while reading Aik as Base64")
	}

	// eventlog: read /opt/trustagent/var/measure-log.json
	tpmQuoteResponse.EventLog, err = ctx.readEventLog()
	if err != nil {
		return nil, errors.Wrap(err, "resource/quote:createTpmQuote() Error while reading event log")
	}

	tpmQuoteResponse.TcbMeasurements.TcbMeasurements, err = ctx.getTcbMeasurements()
	if err != nil {
		return nil, errors.Wrap(err, "resource/quote:createTpmQuote() Error while retrieving TCB measurements")
	}
...
func getTpmQuote(cfg *config.TrustAgentConfiguration, tpmFactory tpmprovider.TpmFactory) endpointHandler {
	return func(httpWriter http.ResponseWriter, httpRequest *http.Request) error {
		...
		ctx, err := NewTpmQuoteContext(cfg)
		if err != nil {
			return &endpointError{Message: "Error creating quote context", StatusCode: http.StatusBadRequest}
		}

		defer ctx.Close()

		tpmQuoteResponse, err := ctx.CreateTpmQuoteResponse(&tpmQuoteRequest)
		if err != nil {
			log.WithError(err).Errorf("resource/quote:getTpmQuote() %s - There was an error collecting the tpm quote", message.AppRuntimeErr)
			return &endpointError{Message: "There was an error collecting the tpm quote", StatusCode: http.StatusInternalServerError}
		}
		...
		if err != nil {
			...
			return &endpointError{Message: "There was an error serializing the tpm quote", StatusCode: http.StatusInternalServerError}
		}
